...
{#snippet selectOption(id: string, nameLocal: string)}
	<button
		class={{
			'block w-full px-4 py-2 text-left text-sm text-gray-700': true,
		...
		{#if open}
			<div
				class="absolute {right
					? 'right-0'
					: ''} z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-hidden"
				role="menu"
