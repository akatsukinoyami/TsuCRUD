<script lang="ts">
	...
	let { links = [{ href: 'dashboard', label: 'Dashboard' }], tables, children } = $props();
	...
	function linkClasses(href: string, mobile: boolean = false) {
		return {
			...
			'text-gray-300 hover:bg-gray-700 hover:text-white': !page?.url?.pathname.startsWith(
				`${href}/`
			)
		};
	}
					...
					<div class="flex h-16 items-center justify-between px-4 sm:px-0">
							...
							<div class="hidden md:block">
								<div class="ml-10 flex items-baseline space-x-4">
									{#each links as { href, label }}
										<a href="{page.url.origin}/{href}" class={linkClasses(href)} aria-current="page"
											>{label}</a
										>
									{/each}
								</div>
							...
							<div class="ml-4 flex items-center md:ml-6">
								<Select
									...
									options={tables.map((table: string) => ({
										id: table,
										name: new TextTransformer(table).capitalize()
									}))}
						...
						<div class="-mr-2 flex md:hidden">
							...
							<button
								...
								aria-expanded="false"
								onclick={() => (mobileMenuState = !mobileMenuState)}
							>
			...
			{#if mobileMenuState}
				<div class="border-b border-gray-700 md:hidden" id="mobile-menu">
					<div class="space-y-1 px-2 py-3 sm:px-3">
						{#each links as { href, label }}
							<a href="{page.url.origin}/{href}" class={linkClasses(href, true)} aria-current="page"
								>{label}</a
							>
						{/each}
